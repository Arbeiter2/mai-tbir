...
def recall(serp, rel=0):
	l = []
	nr_relevant_docs_retrieved = 0
	if rel > 0:
		nr_relevant_docs = rel
	else:
...
# Returns a list of interpolated precisions corresponding to an input list
# of recall points (default: [0.0, ... 1.0] in steps of 0.1
# Use: precision_at_fixed_recall(interpolated_precision(serp), recall(serp))
def precision_at_fixed_recall(interp_precision, recall, points=[]):
	if (len(interp_precision) == 0
	or  len(recall) == 0
	or  any(x < 0.0 or x > 1.0 for x in points)):
		return []
	
	if len(points) == 0:
		points = [float(x)/10 for x in range(0, 11)]

	output = []
	threshold = iter(points)
	val = threshold.next()
	for r, ip in zip(recall, interp_precision):
		if r >= val:
			output.append(ip)
	...
	# all remaining precision values are set to zero
	while len(output) < len(points):
		try:
			output.append(0.0)
			val = threshold.next()
